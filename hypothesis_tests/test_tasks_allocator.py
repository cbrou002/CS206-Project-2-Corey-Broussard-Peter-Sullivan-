...
def tasks_allocator(total, weights, *, minimum=0):
    if total < 0:
...
def test_total_non_negative(total):
    try:
...
def test_minimum_non_negative(minimum):
    try:
        tasks_allocator(10, [1, 2, 3], minimum=minimum)
    except ValueError as e:
        assert "minimum must be non-negative" in str(e)

@given(st.lists(st.integers(), min_size=0, max_size=0))  # empty weights list
def test_weights_provided(weights):
    try:
