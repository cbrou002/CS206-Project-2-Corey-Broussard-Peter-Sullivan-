...
def test_realign_power_shape_mismatch(current, target, damping):
    assume(len(current) != len(target))
    try:
        realign_power(current, target, damping=damping)
    except ValueError as e:
        assert str(e) == "shape mismatch"

@given(st.lists(st.floats(), min_size=1), st.lists(st.floats(), min_size=1), st.floats())
def test_realign_power_empty_allocation(current, target, damping):
    assume(not current)
    try:
        realign_power(current, target, damping=damping)
    except ValueError as e:
...
@given(st.lists(st.floats(), min_size=1), st.lists(st.floats(), min_size=1), st.floats())
def test_realign_power_updates_power_weights(current, target, damping):
    updated = realign_power(current, target, damping=damping)
    for idx in range(len(current)):
