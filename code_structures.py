...
#        # 1. SINGLE COMPARISONS (==, >, <, in, is, etc.)
...
#            return node.operand
#        
#        # 4. CONSTANTS (Python 3.8+: True→False, 0→True, None→True)
...
#        
#        return None  # Complex/chained → fallback NOT(...)
#
...
#        }
#        if not node.orelse:
#            neg = self._negate_simple_len_eq_zero(node.test)
#            if neg is not None:
#                branch_info["else_condition"] = self._pretty(neg)
...
#        visitor.visit(tree)
#
#        # FIXED: use 'visitor' instead of undefined 'v'
#        results = []
#        results.append(f"{'='*60}")
...
#    folder_path = "python_programs"  # or e.g., "/path/to/your/python/files"
#    
...
#        out_file.write(f"Analyzed on: {folder_path}\n")
#        out_file.write("=" * 60 + "\n\n")
#        
...
#            out_file.flush()  # ensure written immediately
#    
#    print(f"Results written to output.txt ({len(py_files)} files analyzed)")
#
#
